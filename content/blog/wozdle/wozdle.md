...
{% blogimage "img/32K-rom.png", "I am not sure he actually said that" %}
...
{% blogimage "img/game-over.gif", "Or is it?" %}
...
{% blogimage "img/try-harder.jpg", "(That makes me hungry)" %}

### Encoding the 12947 word vocabulary

When we just encode the list, we encode all the words *and their order*.

{% digression "A bit of math:" %}
There are 12947! possible orders, and by having a list of words, we choose one. Choosing an order is log2(12947!) bits of information, which, according to Wolfram Alpha, is 158189 bits. It means that we could throw out this information, and save about 20K.
{% enddigression %}
...
{% digression "Fun fact:" %}
One need 3 bytes if the words are more than 65535 apart, and 2 bytes if they are more than 255 apart.

There are only 126 deltas that needs 3 bytes for encoding in the whole vocabulary, the largest one being between 'wytes' (24957107) and 'xebec' (25331875), and is 374768 (a 19 bits value).
{% enddigression %}

At that point, we can take inspiration from UTF-8 and use the first bits to describe how to encode the offsets.

If the value is between 0 and 127, we encode on 7 bits and prefix the bytes with ``0``.
If the value is between 128 and 16383, we encode on 14 bits and prefix the bytes with ``10``.
If the value is between 16384 and 4194304, we encode on 22 bits and prefix the bytes with ``11``.
...
I researched a couple of different optimisations, but didn't find something both simple and efficient.

The most promising one was to add bit at the end of the vocabulary encoding, so words would be even or odd wether they are in the answer list or not. But it is not worth it, as all the offsets would increase, and the cost would be 1848 bytes.
{% enddigression %}
